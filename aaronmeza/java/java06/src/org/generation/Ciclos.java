...
//		System.out.println("Fin de Banorte, su dinero está seguro C: ");
		...
		//Arbolito de Navidad muajaaaaa
		
		
Scanner arbole = new Scanner (System.in); //para introducir cantidad 
		/*
		System.out.print("Ingrese numero de filas a imprimir: ");
		
		int opcionEleg = arbole.nextInt(); //toma la cantidad ingresada y la lamacena en la variable opcionEleg
		opcionEleg = opcionEleg*2-1; // 
		
		//Creamos una matriz
		for(int i = 1; i <= opcionEleg; i+=2) {
			for (int j =1; j <= opcionEleg-i;  j+=2){
				System.out.print(" ");
			}
			for (int k=1; k <= i; k++){
				System.out.print("*");
		...
		for(int k= 1; k<= 3; k++) {	
		for(int i=1;i<=(opcionEleg/2)-1; i++)
				System.out.print(" ");
				for(int j=1; j<=3; j++)
	             System.out.print("*");
				System.out.println();
